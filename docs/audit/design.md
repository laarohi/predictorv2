...
- **`DESIGN-2` — match-detail header crammed against the nav** (Luke's example 1). The desktop `.pn-md-top` / mobile `.pn-mm-only` used `margin-top:-28px`, which *over*-pulled past the body's 14px top padding so the navy hero overlapped up into the red sub-strip — the crammed look. **First attempt (`f14c5b0`) was wrong:** a positive top margin pushed the block down and exposed a *cream* gap, making the navy hero look detached from the chrome. **Corrected (`4dcb7e1`)** per Luke's note ("the gap should match the navy colour"): the breathing room must be navy, not cream. The container `.pn-md-top` has no background, so margin/padding on it reveals the cream `.pn-body`; gave it `background: var(--ink)` + `padding-top:12px` (navy breathing room above both columns) and `margin-top:-14px` (cancels exactly the 14px body padding → flush under the red strip, no cream seam, no overlap). Mobile `.pn-mm-only` restored to `-14px` (flush navy bar at the top, no cream band; the old `-28px` clipped the bar's top 14px). Verified desktop + mobile with element crops.
